...
const TeacherCard = ({ id, image, name, ...socials }) => (
  <div className="group block">
    <div className="relative w-full h-120 mb-5 border-card rounded-2xl overflow-hidden shadow-[6px_6px_0px_rgba(38,38,38,1)]">
      ...
      <Link to={`/team/${id}`} className="block w-full h-full">
        <img
          src={image}
          alt={name}
      ...
      {/* social media icons */}
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-3 opacity-0 translate-y-10 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500 z-10">
    ...
    {/* Button con el nombre del profesor */}
    <Link
      to={`/team/${id}`}
      className="block bg-white text-center border-card py-3 text-2xl font-medium rounded-xl font-display transition-all duration-300 group-hover:-translate-y-1 group-hover:bg-primary"
    >
      {name}
    </Link>
  </div>
